...
indiv_list = open('./../../individual_list.txt','r').readlines()
...
for x,mutation in enumerate(indiv_list):
	newaa = mutation.strip(';\n')
	indiv.write(newaa+';\n')
...
while 'ST_' in output_qstat:
	print 'Waiting for all Indiv jobs to finish'
	time.sleep(10)
	check_qstat = subprocess.Popen('qstat',stdout=subprocess.PIPE)
	output_qstat = check_qstat.stdout.read()
...
if os.path.isfile('./Repair/Interaction_AnalyseComplex_'+name+'.fxout'): 
	...
	g.write('\n')
dirs = sorted(glob.glob('./Runs/Indiv/*'))
