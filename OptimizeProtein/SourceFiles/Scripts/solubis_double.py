...
for mol in Mols:
	...
	yasara.run('LoadPDB '+startingDir+'/Repair/RepairPDB_'+name+'.pdb')
	...
	for line in f[1:]:
		...
		for x,aa in enumerate(stretch):
			...
			AA = aa_dict_1to3[aa]
			contactnums = yasara.run('ListRes all with distance<6 from '+AA+' '+str(index_mutation)+' and Mol '+mol+',Format=RESNUM')
			contactres = yasara.run('ListRes all with distance<6 from '+AA+' '+str(index_mutation)+' and Mol '+mol+',Format=RESNAME1')
			for gate in gatekeepers:
				...
				combos = []
				combos.append(mutation)
				for y,contactnum in enumerate(contactnums):
					for aanew in AAlist:
						if aanew != contactres[y]:
							if index_mutation != str(contactnums[y]):
								combos.append(mutation+'_'+contactres[y]+mol+str(contactnums[y])+aanew)
...
while 'SBD_' in output_qstat:
        print 'Waiting for all SolubisDouble jobs to finish'
        time.sleep(10)
        check_qstat = subprocess.Popen('qstat',stdout=subprocess.PIPE)
        output_qstat = check_qstat.stdout.read()
...
g = open('SummarySolubisDouble.txt','w')
...
if os.path.isfile('./Repair/Interaction_AnalyseComplex_'+name+'.fxout'): 
	...
	g.write('\n')
dirs = sorted(glob.glob('./Runs/SolubisDouble/*'))
