...
gatekeepers = ['A','C','D','E','F','G','H','I','K','L','M','N','P','Q','R','S','T','V','W','Y']
name = pdb.split('/')[-1].split('.')[0].split('_')[-1]
print name
agadirs = []
totals = []
total_pdb = 0
os.chdir('Runs/SolubisMild')
...
while 'SB_' in output_qstat:
        print 'Waiting for all SolubisMild jobs to finish'
        time.sleep(10)
        check_qstat = subprocess.Popen('qstat',stdout=subprocess.PIPE)
        output_qstat = check_qstat.stdout.read()
...
g = open('SummarySolubisMild.txt','w')
...
if os.path.isfile('./Repair/Interaction_AnalyseComplex_'+name+'.fxout'): 
	...
	g.write('\n')
dirs = sorted(glob.glob('./Runs/SolubisMild/*'))
