...
summaryfinal = open('FinalSummarySolubis.txt','w')
sumshort = open('FinalSummarySolubisShort.txt','w')
summaryfinal.write('Pdb\tMol\tRes\tTANGO\tAPRnumber\tResnumTANGO\tResnumFoldX\tMutation\tddG_R\tdTANGO_R\tddG_P\tdTANGO_P\tddG_K\tdTANGO_K\tddG_E\tdTANGO_E\tddG_D\tdTANGO_D\n')
sumshort.write('Pdb\tMol\tMutation\tddG\tdTANGO\n')
for mapline in mapping:
	...
	for line in sumlines:
		...
		for lala,mut in enumerate(muts):
			sumshort.write(name+'\t'+mol+'\t'+muts[lala]+'\t'+ddGs[lala]+'\t'+dTANGOs[lala]+'\n')
		summaryfinal.write(name+'\t'+mol+'\t'+aa+'\t'+tangores+'\t'+APRnumber+'\t'+index_agadir+'\t'+index_mutation_foldX+'\t'+mutation+'\t'+ddG_R+'\t'+dtango_R+'\t'+ddG_P+'\t'+dtango_P+'\t'+ddG_K+'\t'+dtango_K+'\t'+ddG_E+'\t'+dtango_E+'\t'+ddG_D+'\t'+dtango_D+'\n')
summaryfinal.close()
sumshort.close()
